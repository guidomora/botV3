...
describe("GIVEN DeleteReservationUseCase", () => {
  ...
  describe("WHEN deleteReservation is called", () => {
    ...
    it("SHOULD return a message if the date is not found", async () => {
      (googleSheetsService.getDateIndexByData as jest.Mock).mockResolvedValue(
        -1,
      );

      const result = await deleteReservationUseCase.deleteReservation(
        reservation,
      );
    ...
    it("SHOULD delete the reservation when there is only one date", async () => {
      ...
      (googleSheetsService.getDatetimeDates as jest.Mock).mockResolvedValue([
        "date",
      ]);
    ...
    it("SHOULD throw an error and log it", async () => {
      ...
      (googleSheetsService.getDateIndexByData as jest.Mock).mockRejectedValue(
        errorMock,
      );

      await expect(
        deleteReservationUseCase.deleteReservation(reservation),
