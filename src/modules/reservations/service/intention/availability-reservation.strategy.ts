...
export class AvailabilityStrategy implements IntentionStrategyInterface {
  ...
  async execute(
    ...
    if (!aiResponse.date) {
      ...
      const availabilityResponse = await this.aiService.dayAvailabilityAiResponse(
        availability,
        history,
      );
      ...
      const availability = await this.dateService.getDayAndTimeAvailability(
        aiResponse.date,
        aiResponse.time,
      );

      const availabilityResponse = await this.aiService.dayAndTimeAvailabilityAiResponse(
        availability,
        history,
        aiResponse.time,
      );
